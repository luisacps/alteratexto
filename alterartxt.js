...
function inputTxt(formulario) {
    const i = formulario.querySelector("input[type=text]");
...
function inserirTarefa(evento) {
    ...
    const texto = inputTxt(evento.target);
    if (texto == "") return;
    const tarefa = novaTarefa(t);
...
function novaTarefa(t) {
    ...
    tarefa.append(adicionarCheckbox());
    tarefa.append(texto + " ");
    tarefa.append(criarLixeira());
	tarefa.append(criaEAtualiza());
    return tarefa;
}
